...
function DiscordLib:Window(text)
    ...
    if #hexColor == 6 and tonumber(hexColor, 16) ~= nil then
        ...
        -- Adjusting color components to make the color darker
        local colorDark = {}
        colorDark.R = math.max(r - 4, 0)
        colorDark.G = math.max(g - 4, 0)
        colorDark.B = math.max(b - 4, 0)

        local colorServ = {}
        colorServ.R = math.max(r - 2, 0)
        colorServ.G = math.max(g - 2, 0)
        colorServ.B = math.max(b - 2, 0)

        local colorChan = {}
        colorChan.R = math.max(r - 6, 0)
        colorChan.G = math.max(g - 6, 0)
        colorChan.B = math.max(b - 6, 0)

        print(colorDark.R, colorDark.G, colorDark.B)
        print(colorServ.R, colorServ.G, colorServ.B)
        print(colorChan.R, colorChan.G, colorChan.B)
...
local injectTab = window:Server("Inject", "")

local codeSection = playerTab:Channel("Code")

-- Create a Textbox for inputting Lua code
local codeInput = codeSection:Textbox("Code", function(code)
    -- Execute the inputted Lua code
    local success, err = pcall(loadstring(code))
    
    -- Check if execution was successful
    if not success then
        warn("Error executing Lua code:", err)
    end
end)
